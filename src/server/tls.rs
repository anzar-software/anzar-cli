...
pub fn configure_tls(cert: String, key: String) -> Result<rustls::ServerConfig, Error> {
    ...
    if cert_chain.is_empty() {
        return Err(Error::Internal(InternalError::Tls { path: cert.clone() }));
    }
    ...
    let key = private_key(key_reader)?
        .ok_or_else(|| Error::Internal(InternalError::Tls { path: cert.clone() }))?;

    // Build TLS config
    ServerConfig::builder()
        .with_no_client_auth()
        .with_single_cert(cert_chain, key)
        .map_err(|_| Error::Internal(InternalError::Tls { path: cert.clone() }))
}
