...
use shared::error::{AuthError, ConflictReason, CoreError, CredentialField, ForbiddenReason};

use crate::{
    error::{Error, ErrorResponse, Result},
    http::{
        cookies,
        extractors::{AppStateExtractor, SessionExtractor, ValidatedQuery},
...
pub async fn login(
    ...
    let (user, account, account_status, attempts) = app_state
        .auth_service
        .authenticate_user(&req, device_cookie)
        .await?;
    ...
    if auth_config.email.verification.required && !account.verified {
        ...
        return Err(CoreError::from(AuthError::AccountNotVerified).into());
    }
    ...
    match account_status {
        AccountStatus::Active => {
            ...
            match &app_state.configuration.auth.strategy {
                AuthStrategy::Session(..) => app_state
                    .session_service
                    .issue_session(user_id, full_permissions)
                    .await
                    .map(|token| {
                        session.clear();
                        session.renew();
                ...
                AuthStrategy::Jwt(jwt) => app_state
                    .session_service
                    .issue_jwt(user_id, full_permissions)
                    .await
                    .map(|tokens| {
                        session.clear();
                        session.renew();
        ...
        AccountStatus::Suspended => Err(CoreError::from(ForbiddenReason::AccountSuspended).into()),
        _ => {
            throttle::delay(attempts as u32).await;
            return Err(CoreError::from(AuthError::InvalidCredentials {
...
pub async fn register(
    ...
    let result = async {
        match app_state.auth_service.create_user(req.into_inner()).await? {
            CreateUserOutcome::Created(user) => {
                ...
                if email_config.required {
                    let token = app_state
                        .auth_service
                        .create_verification_email(user_id)
                        .await?;
                    let link = format!(
                        "{}/email/verify?token={}",
            ...
            CreateUserOutcome::AlreadyExists => {
                Err(CoreError::from(ConflictReason::AlreadyExists {
...
async fn request_password_reset(
    ...
    let result = async {
        ...
        // 4.
        let expiring_link = app_state
            .auth_service
            .insert_password_reset_token(user_id)
            .await?;
...
async fn submit_new_password(
    ...
    app_state.session_service.logout_all(user_id).await?;
