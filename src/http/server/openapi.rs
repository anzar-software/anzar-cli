...
use crate::domain::model::{ExpiringLink, LoginRequest, ResetPasswordRequest, Session, User};
...
pub fn swagger_service(configuration: &AnzarConfiguration) -> SwaggerUi {
    ...
    let cookie_name = match configuration.clone().auth.strategy {
        crate::config::AuthStrategy::Session(session_config) => session_config.name,
        crate::config::AuthStrategy::Jwt(_) => String::new(),
    };
